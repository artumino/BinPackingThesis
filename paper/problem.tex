...
In the literature there are some mathematical formulations tackle the concept of area support (and in some cases vertex support). In \citefullauthor{elhedhli2019three} a SOCP formulation of the support constraint was used but was limited to the problem of spacing between layers with one of the layers being fixed in position, in our case a similar formulation would lead to a non-linear support constraint.
By introducing a discretization over the XY-plane a linear version of the constraint can be then formulated, a similar formulation, although applied to all the space, was proposed in \citefullauthor{}
...
And finally we can introduce a feasibility constraint which ensures that every item that isn't on the ground is supported by other items placed beneath it by at least $\beta_s$ times its area, which corresponds to condition \ref{support:area_support} of the pratical constraint of vertical support.
\begin{eqnarray}
