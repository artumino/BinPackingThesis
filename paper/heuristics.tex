...
Given a bin $b \in s.B$ we can then define his supporting structure $s_b$ as follows
\begin{itemize}
...
Notice that two separate sets are present inside $s_b$ since adding and accessing items in $s_b.J$ has time complexity of $O(1)$ given an underlying implementation as hashset while mantaining $s_b.T$ usually has a time complexity of $O(log(|s_b.J|))$.
...
\begin{definition}[Insertion]
    Given a state $s$ and a tollerance $\beta_s$ we define an insertion or placement $P$ a tuple $(b, I^\prime)$ where $b$ is a bin and $I^\prime$ is a set of items that are going to be packed in $b$ such that, $I^\prime \subseteq s.U \land \exists (z \in \mathbb{Z} : \forall i \in I^\prime |z_i - z| \le \beta_s)$
\end{definition}

\begin{observation}
    Given $s$ and $P = (b, \emptyset)$ where $b \notin s.B$, $P$ is insertion which will open bin $b$ in $s$.
\end{observation}
...
\begin{proposition}
    A state $s^\prime$ derived by committing a feasible insertion $P$ to state $s$, is feasible.
\end{proposition}
