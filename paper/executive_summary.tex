...
This wrapping procedure requires that the ammount of unused space between items is minimal and measures this property with a metric called cage ratio.
Cage Ratio is the ratio between the volume of the packed items inside a bin and the volume of the cuboid which surrounds them called cage.
The cage has the same base as the bin and the height equalt to the heighest packed item inside the bin.
...
Since the 3D-BPP is the generalization of the one-dimensional bin packing problem, it is NP-Hard (\cite{martello2000three}).
...
\subsection{Figures}
\label{sec:figures}
To include Figures in your text you can use \texttt{TikZ} for high-quality hand-made figures,
...
It is suggested to use the BibTeX package and save the bibliographic references
in the file  \verb|literature.bib|.
