...
Since an exaustive search isn't feasible, an heuristic search is conducted by combining a beam search algorithm described in section \ref{sec:beamsearch} and a constructive heuristic described in section \ref{sec:support_planes}.
...
Notice that two separate sets containing the items packed in $b$ are present inside $q_b$ but adding and accessing items in $q_b.J$ has time complexity of $O(1)$ given an underlying implementation as hashset while mantaining $q_b.T$ usually has a time complexity of $O(log(|q_b.J|))$.
...
The underlying heuristic will also use a scoring mechanism to select the best instertions for a given class of heights in order to avoid having too many states to sort.

Given a set of items $I$ and a tollerance $\beta_s$ we can introduce an algorithm to group them by height and produce a set $G$ of tuples $(h, I^\prime)$ where $h$ is the height of the group and $I^\prime$ is the set of items grouped as in algo. \ref{algo:group_by_height}.
...
Given a state $s$ and an insertion $p$ where each packed item $i \in p.I$ in bin $b$ has $z_i$ within tollerance of $z$ and the minimum height for the considered bin $q_b.z_{min}$.
