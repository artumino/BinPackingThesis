...
Every item which is placed in the bin has the ability to either generate a new support plane or be part of the supporting items of other planes. 
Items that are placed above a particular plane are considered obstacles and are added to the $I_{upper}$ set.
Since insertions always happen in the lowest possible planes, the set of obstacles of planes where new insertions can be made are usually composed of items that have their top face above the $z$ of the evaluated plane. %TODO: maybe update this
When evaluating a new possible insertion, given a set of items to place $I$, support planes selects the first feasible insertion starting from the lowest plane by using a modified version of Extreme Point in two dimensions (introduced in \cite{crainic2008extreme}).

The Extreme Point algorithm consists in the evaluation of placements of rectangles in a plane based on a set of reference points. 
...
In our verion of the algorithm the extreme points of each item are introduced without projection to increase the likelyhood of evaluating placements that verify the support constraint.
...
If a placement in a given reference point can't be made, it is also evaluated if the item can be rotated to fit.
...
To address the problem we evaluate four instances of extreme points with each instance having a different coordinate change applied to every item which moves the origin of the plane to each corner of the bin.
This addition is based on similar approaches from the literature where it usually used to more uniformly fill the space (ex. \citeauthor{GAJDA2022102559}) and was verified to yeild better cage ratio results in our internal testing.
%TODO: Check the above
Elements from the collection of obstacles for a plane are considered as fixed placements already made with their extreme points already added to the reference point collection, this allows us to also evaluate placements near other items from different planes.
When checking for a possible placement over a plane, the feasibility check defined in \cref{sec:problem_state:feasibility} can be used to avoid selecting unfeasible insertions.
A graphical rappresentation of a support plane is shown in \cref{fig:support_planes}, when a bin is opened the only support plane available is the one on the ground.

%TODO: Proporre la versione ibrida maxrect/extreme points?
%TODO: Separare parte extreme point da parte support plane
%TODO: Spiegare concetto di obstacles con magari immagine
